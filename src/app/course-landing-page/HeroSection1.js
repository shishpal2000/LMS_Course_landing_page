...
const HeroSection1 = ({ course_id,instructor_intro_text,instructor_name
,  title, tag_line, logos, image ,batch_start_date,batch_start_date_text,application_deadline_text
    ...
    <div className="w-full relative ">
      ...
      <div className="w-full h-full absolute top-0 left-0 flex pb-[10%]">
        <div className="w-full md:w-[56%] flex flex-col px-[5%] py-[2%] items-center md:items-start pt-[7%] mm:pt-[5%] md:pt-0 justify-center">
          ...
          />
          <p className="text-gray-700 text-3xl md:text-4xl lg:text-5xl xl:text-7xl font-semibold mb-0 mt-[6%] text-center md:text-start">
          ...
          <p className=" md:block text-gray-700 text-[16px] mm:Text-xs md:text-sm lg:text-base xl:text-xl mt-[8%] font-semibold text-center md:text-start lg:pb-16 xl:pb-20">
            {tag_line}
            <br/>
            <p className=" text-lg md:text-xl xl:text-3xl mb-2">₹ {course_id.price}</p>
          </p>
          ...
          <div className="w-full bg-[#525FE1] flex px-[5%] py-[2%] lg:rounded-2xl xl:rounded-[32px] 2xl:rounded-[48px] 2xl:mt-10 justify-between items-center">
            ...
            <Link
                      href={`/loginpage?course_id=${course_id
                        ?._id}&price=${course_id?.price?.amount}&is_paid=${course_id?.is_paid}&icon_url=${logos}`}
